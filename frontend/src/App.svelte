<script lang="ts">
  ...
  async function openFile() {
    error = "";
    try {
      // Dynamic import: in plain vite dev mode the wailsjs/ folder may not exist.
      const App = await import("../wailsjs/go/main/App").catch(() => null);
      const runtime = await import("../wailsjs/runtime/runtime").catch(() => null);
      if (!App || !runtime) {
        throw new Error("Wails bindings not available — running in plain vite dev. Loaded bundled sample.");
      }
      ...
      if (!path) return;
      // @ts-expect-error — Wails-generated types use doc.FictionBook; we treat it as our FictionBook.
